...
void main(){
    ...
    float dist = length(u_CameraPos.xz - fs_Pos.xz);
    dist = smoothstep(150, 200, dist);


    float blurMultiplier = clamp(u_idleTime * 2.f, 0, 5);

    float otherDist = length(u_CameraPos - fs_Pos)/100;
    otherDist = abs(dot(u_CameraLook, fs_Pos - u_CameraPos));
    float focalLength = u_nearestBlockDist;

    float focalRange = u_nearestBlockDist * 5 * (1/blurMultiplier);
    float dofBlurAmt = min(1.f, abs(otherDist- focalLength)/focalRange);



    out_Col = vec4(vec3(dist, dofBlurAmt, 0), 1.f);
