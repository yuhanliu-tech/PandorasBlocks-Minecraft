...
void main()
{
    ...
    if(approxEqual(vs_BlockSFX[3], 1.f)) {//It's water! Max height is 128
        offsetPos.y += displaceWaves(offsetPos.xz);
    } else if (approxEqual(vs_BlockSFX[0], 2.f)) {
        offsetPos.z += displaceGrass(offsetPos.xy);
    } else if (approxEqual(vs_BlockSFX[0], 4.f)) {
    ...
    gl_Position = u_lightSpaceMatrix * modelposition;// gl_Position is a built-in variable of OpenGL which is
