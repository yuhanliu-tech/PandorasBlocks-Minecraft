...
//5
#define TERRAIN_DRAW_RADIUS 3// 3 AND 5
//6
...
private:
    ...
    // multithreading
    // every worker that populates chunks w block type data has access to:
    // bucket into which all block type workers feed their output
    unordered_set<Chunk*> m_chunksThatHaveBlockData;
    QMutex m_chunksThatHaveBlockDataLock;
    std::vector<chunkVBO> m_chunksThatHaveVBOs;
...
//    //helper function to check edges of chunks
...
public:
    ...
    // helper to create neighbors when new chunks added
    void updateChunkNeighbors(int, int);
    ...
    // checks whether new chunk should be added to Terrain based on player's proximity to edge
    void terrainExpansion(const glm::vec3 &playerPos);
    ...
    void CreateTestScene(ShaderProgram *shaderProgram);

    void drawTerrain(int minX, int maxX, int minZ, int maxZ, ShaderProgram *shaderProgram);

    // multithreading functions
    void multithreadedWork(glm::vec3 currPos, glm::vec3 prevPos, float DT);
    void tryExpansion(glm::vec3 currPos, glm::vec3 prevPos);
    QSet<int64_t> terrainZonesBorderingZone(glm::ivec2 zoneCoords, unsigned int radiusInZoneSc, boolean onlyCircumference);
    ...
    void createInitialChunks(glm::vec2 pos);
