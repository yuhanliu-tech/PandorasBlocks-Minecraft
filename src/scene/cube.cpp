...
static const int CUB_IDX_COUNT = 36; //6 faces, 2 triangles each, 3 verts per triangle
static const int CUB_VERT_COUNT = 24; // 6 faces, 4 verts each

static const int STANDEE_IDX_COUNT = 24; //4 faces, 2 triangles each, 3 verts per triangle
static const int STANDEE_VERT_COUNT = 16; // 4 faces, 4 verts each.

glm::vec4 GetCubeNormal(const glm::vec4& P) //May need to modify for standees :(
{
    int idx = 0;
    float val = -1;
    for(int i = 0; i < 3; i++){
...
//when filling the arrays used to hold the vertex and index data.  //FOR CUBES
