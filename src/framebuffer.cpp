...
FrameBuffer::FrameBuffer(OpenGLContext *context,
    ...
    : mp_context(context), m_frameBuffer(-1),
      ...
      m_width(width), m_height(height), m_devicePixelRatio(devicePixelRatio), m_created(false), isDepthBuffer(false)
{}

FrameBuffer::FrameBuffer(OpenGLContext *context,
                         unsigned int width, unsigned int height, unsigned int devicePixelRatio, bool isDepth)
    : mp_context(context), m_frameBuffer(-1),
      m_outputTexture(-1), m_depthRenderBuffer(-1),
      m_width(width), m_height(height), m_devicePixelRatio(devicePixelRatio), m_created(false), isDepthBuffer(isDepth)
{}
...
void FrameBuffer::create() {
    ...
    if(isDepthBuffer) {
        //FOR SHADOW MAPPING ONLY *************************:
        mp_context->glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, m_width * m_devicePixelRatio, m_height * m_devicePixelRatio, 0, GL_DEPTH_COMPONENT, GL_FLOAT, (void*)0);
    ...
    if(isDepthBuffer) {
        //FOR SHADOWMAPPING ONLY //LATER CHANGE, may achieve same effect
        GLenum drawBuffers[1] = {GL_NONE};
        mp_context -> glDrawBuffers(1, drawBuffers); //figure out later possibly skipable
